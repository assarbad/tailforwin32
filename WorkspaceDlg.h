 ...
 * 
 * $Id: WorkspaceDlg.h,v 1.5 2004/04/07 18:11:14 paulperkins Exp $
 * 
 */
...
//  void OnRButtonDown (UINT nFlags, CPoint point);
  ...
  // Most of these methods should not be in the dialog!!!
  ...
  BOOL DeleteWorkspaceFile (CWorkspaceItem* pItem);

  BOOL RemoveWorkspace (CWorkspace* pWorkspace);

  CWorkspaceItem* FindFile (CWorkspace* pWorkspace, char* pszFilename);

  BOOL FileLoaded (char* pszFilename);

  CWorkspace* GetWorkspace (CWorkspaceItem* pWorkspaceItem);

  void ActivateItem (CWorkspaceItem* pItem);
  CWorkspace* AddWorkspace (char* pszName);
  CWorkspace* AddWorkspace (char* pszName, CWorkspaceDoc* pDoc);
  BOOL AddNewWorkspace (void);

  BOOL UpdateWorkspace (CWorkspace* pWksp);
...
protected:
	...
	afx_msg void OnWorkspaceCloseWorkspace();
