 ...
 * 
 * $Id: TallyWindow.cpp,v 1.9 2006/01/23 14:12:17 paulperkins Exp $
 * 
 */
...
//  SetExStyles ();
  ...
  ListView_SetExtendedListViewStyleEx (pList->GetSafeHwnd (), 0, LVS_EX_DOUBLEBUFFER);
...
void TallyWindow::SetExStyles()
{
  int styles = (int)::SendMessage(GetSafeHwnd (), 
        (int) LVM_GETEXTENDEDLISTVIEWSTYLE, 0,0);
    styles |= LVS_EX_DOUBLEBUFFER | LVS_EX_BORDERSELECT;
    ::SendMessage(GetSafeHwnd (), 
        (int) LVM_SETEXTENDEDLISTVIEWSTYLE, 0, (int) styles);
}