 ...
 *
 * $Id: TailView.cpp,v 1.35 2006/05/19 12:12:44 paulperkins Exp $
 * 
 */
...
int CTailView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
  ...
  int aiBarParts[] = {135, 360, 360, -1};   // Bug #1019400 - Make first segment larger.
  ...
  if (!m_pSettings->GetGotFont())
  {
    ...
    cf.crTextColor = m_pSettings->GetFontColour ();
    m_ctlEdit.SetDefaultCharFormat (cf);
  }
  else
  {
    SetViewFont (&m_pSettings->GetLogFont(), m_pSettings->GetFontColour ());
  }
...
void CTailView::OnSize(UINT nType, int cx, int cy) 
{
  ...
  m_ctlEdit.MoveWindow (0, 0, cx, cy - 25);	
...
//  strcpy (m_szTailFile, pDoc->m_strTailFile); 
  strcpy (m_szTailFile, pDoc->GetPathName ());
...
//    m_pThread = AfxBeginThread ((AFX_THREADPROC) EventLogThread, (LPVOID) &m_stELParams);
  ...
  GetParentFrame()->SetWindowText (m_szTailFile);
}
...
//      pView->m_ctlEdit.ReplaceSel (szLine, FALSE);
    ...
    if (pstKeywords)
    {
      lNumKeywords = pstKeywords->dwNumKeywords;
    }
    ...
    if (lNumKeywords)
    {
      ...
      for (int iIndex = 0; iIndex < lNumKeywords; iIndex++)
      {
        ...
        {
          // Hmmm - rework this for case-insensitive comparison (bug #1473486).
          while (pszToken = strstr (pszPtr, pstKeywords[iIndex].szKeyword)) // Warlock, using pszPtr, fix multiple keywords per line
          {
            ...
            // Get the start and end of the current keyword.
            lSelStart -= (strlen (szLine) - (pszToken - pszStart) - 1); // Warlock was right (sorry!)
            lSelEnd -= (strlen (szLine) - ((pszToken - pszStart) + strlen (pstKeywords[iIndex].szKeyword)) - 1); // Warlock, - 1

            // Get the start and end of the line for the background shading.
            if (lLineSelStart == 0)
            {
      ...
      // Set the default font colour (Bug #900921).
 /*     pView->m_ctlEdit.SetSel (lLineSelStart, lLineSelEnd - 1);

      stCF2.cbSize = sizeof (stCF2);
      stCF2.dwMask = CFM_COLOR;
      stCF2.crTextColor = pView->m_pSettings->GetFontColour (); // Bug #900921.
  ...
  void)
{
  ...
  if (lKeywordCount)
  {
    ...
    pstNewKeywordList->dwNumKeywords = lKeywordCount;
  ...
  void)
{
  ...
  if (m_lNumKeywords = m_pSettings->GetNumKeywords())
  {
    ...
    m_pstKeywordList->dwNumKeywords = m_lNumKeywords;
...
//  theApp = (CTailApp*) AfxGetApp ();
  ...
  stFontDlg.m_cf.rgbColors = m_pSettings->GetFontColour ();
  ...
  if (IDOK == stFontDlg.DoModal ())
  {
    ...
    stCF.dwMask |= CFM_UNDERLINE | CFM_BOLD | CFM_ITALIC | CFM_FACE | CFM_COLOR;
    stCF.crTextColor = stFontDlg.GetColor ();
    ...
    m_pSettings->SetFontColour (stCF.crTextColor);

    // Bug #900921.
/*    memset (&stCF, 0, sizeof (stCF));
    
    stCF.cbSize = sizeof (stCF);
    stCF.dwMask = CFM_COLOR;
    stCF.crTextColor = m_pSettings->GetFontColour (); 

    ::SendMessage(m_ctlEdit, EM_SETCHARFORMAT, (WPARAM)SCF_ALL, (LPARAM)&stCF); */
...
//    AfxMessageBox ("(Debug Warning) Could Not Kill Thread!");
    LogMessage ("(Debug Warning) Could Not Kill Thread!");
  }
...
void CTailView::OnUpdatePaused(CCmdUI* pCmdUI) 
{
  if (pCmdUI->m_pMenu)
    pCmdUI->SetCheck (this->m_bPaused ? MF_CHECKED : MF_UNCHECKED);		
}
